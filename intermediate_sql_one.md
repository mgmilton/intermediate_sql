...
Serial auto increments each ID.
...
The below four selections all return four.

```sql
SELECT COUNT(name) FROM items;
SELECT COUNT(revenue) FROM items;
SELECT COUNT(course) FROM items;
```
...
```sql
SELECT max(course), min(course) FROM items;
```
Max returns "side" and min returns "main".

1. What's the total revenue for all `main` courses?
```sql
SELECT revenue FROM items WHERE course='main'
```
...
* Write a query that pulls all the category names for `arugula salad`.
  ...
  ```sql
  SELECT items.name, categories.name, seasons.name FROM items INNER JOIN seasons ON items.season_id = season.id INNER JOIN categories ON seasons.id = categories.id WHERE items.name = "arugula salad";
  ```
...
My guess is that it will remove the null values.
```sql
SELECT *
FROM items i
...
```sql
INSERT INTO items (name, revenue, season_id)
        VALUES ('mike dao', 600, NULL),
                ('ali schlereth', 150, NULL),
                ('ilana corson', 700, NULL);

SELECT * FROM items; /* for proof */
```
